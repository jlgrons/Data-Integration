...
if(jj == 2){
  N2  = c(357,  391,  774,  213,  232,   43,  121,  110,  382,  284,  294,  563,  278,  418,
  ...
  N2  = c(176, 207, 185, 109, 116,  47, 124, 114, 384, 135, 302, 142, 279, 212, 198,
          106,  99, 107, 139,  96, 120, 173,  58,  38, 276, 111, 143, 242,  88, 172,
          61, 377, 325, 185, 280, 272, 154, 160, 112, 124, 2634, 2895,  51, 115,  75,
          24, 195, 225)
  N1 = c(357,  391,  774,  213,  232,   43,  121,  110,  382,  284,  294,  563,  278,  418,
         395,  203, 104,  212,  138,  196,  122,  175,   56,   39,  561,  116,  148,  231,   89,
         168,  116, 1172,  706,  204,  288,  254,  314,  162, 442,  394, 2635, 1456,  101,  232,
         70,   25,  196,  676)
  N1 = N1[1:K_org]
...
for(i in ((kk-1)*numb + 1):((kk-1)*numb + numb)  ){
  ...
  # bs.res = parSapply(cl, 1:2,
  #                    function(kk){
  #                      if(kk == 1){res = outer_bound(bb, mom.o, data[, 'Grp1_Ev'], data[, 'Grp1_Ev'] + data[, 'Grp2_Ev'],
  #                                                    data[, 'Grp1_Sz'], data[, 'Grp2_Sz'], CI.dsl, mesh.size = 0.01)}else{
  ...
  # Run the code without parallelization
  res_1 = outer_bound(bb, mom.o, data[, 'Grp1_Ev'],
                            data[, 'Grp1_Ev'] + data[, 'Grp2_Ev'],
                            data[, 'Grp1_Sz'], data[, 'Grp2_Sz'],
                            CI.dsl, mesh.size = 0.01)
  res_2 = lower_bound(bb, mom.o, data[, 'Grp1_Ev'],
                            data[, 'Grp1_Ev'] + data[, 'Grp2_Ev'],
                            data[, 'Grp1_Sz'], data[, 'Grp2_Sz'],
                            CI.dsl, mesh.size = 0.01)
  
  res_CI = rbind(res_1, res_2)
  # ptm2 <- proc.time() - ptm
  #res_CI = do.call(rbind, bs.res)
...
write(t(CIs.all),file =
        paste0(kk, 'balan-', jj, 'CI-', K_org,  '-alpha-', alpha, '-beta-', beta,
               '-baseR-', base_rate, ".txt"),
...
write(t(tot),file =
        paste0(kk, 'balan-', jj, 'TOT-', K_org,  '-alpha-', alpha, '-beta-', beta,
               '-baseR-', base_rate, ".txt"),
